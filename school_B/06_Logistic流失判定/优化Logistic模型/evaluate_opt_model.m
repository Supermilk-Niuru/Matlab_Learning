function m = evaluate_opt_model(Y, p, outDir)
% EVALUATE_OPT_MODEL 优化模型评价：混淆矩阵 / 评价指标 / ROC 曲线与 AUC
%   判定规则：P >= 0.5 判为流失（正类=1，流失），否则判为未流失（负类=0）。
%   使用与原始 Logistic 模型完全相同的测试集与评价口径。
%   混淆矩阵（以"流失"为正类）：
...
%
%   输出文件（保存在 outDir）：
%       Optimized_model_evaluation.xlsx   Sheet1 混淆矩阵，Sheet2 评价指标（含 AUC）
%       Optimized_ROC_curve.png           ROC 曲线图
%   返回：
%       m —— 结构体：Accuracy / Precision / Recall / F1 / AUC / n
%
...
%% ---------- 4) 保存 Optimized_model_evaluation.xlsx ----------
rowLbl = {'预测为流失'; '预测为未流失'};
...
evalFile = fullfile(outDir, 'Optimized_model_evaluation.xlsx');
...
%% ---------- 5) 绘制并保存 Optimized_ROC_curve.png ----------
...
ylabel('真阳性率 TPR（灵敏度）');
title('优化Logistic模型（13变量）ROC 曲线');
legend(sprintf('ROC (AUC = %.4f)', AUC), '随机基准', 'Location', 'southeast');
grid on; box on;
rocFile = fullfile(outDir, 'Optimized_ROC_curve.png');
saveas(gcf, rocFile);
close(gcf);
...
%% ---------- 6) 返回指标结构体（供模型对比） ----------
m = struct('Accuracy', Accuracy, 'Precision', Precision, ...
           'Recall', Recall, 'F1', F1, 'AUC', AUC, 'n', n);
