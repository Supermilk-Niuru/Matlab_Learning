function evaluate_RF(Y, p, outDir)
% EVALUATE_RF 随机森林流失判定模型评价：混淆矩阵 / 评价指标 / ROC 曲线与 AUC
%   判定规则：P >= 0.5 判为流失（正类=1，流失），否则判为未流失（负类=0）。
%   与问题2 Logistic 模型使用完全相同的评价口径与指标，便于论文对比。
%   混淆矩阵（以"流失"为正类）：
...
%
%   输出文件（保存在 outDir）：
%       RF_model_evaluation.xlsx   Sheet1 混淆矩阵，Sheet2 评价指标（含 AUC）
%       RF_ROC_curve.png           ROC 曲线图
%
...
%% ---------- 4) 保存 RF_model_evaluation.xlsx（混淆矩阵 + 指标） ----------
rowLbl = {'预测为流失'; '预测为未流失'};
...
evalFile = fullfile(outDir, 'RF_model_evaluation.xlsx');
...
ylabel('真阳性率 TPR（灵敏度）');
title('随机森林流失判定模型 ROC 曲线');
legend(sprintf('ROC (AUC = %.4f)', AUC), '随机基准', 'Location', 'southeast');
grid on; box on;
rocFile = fullfile(outDir, 'RF_ROC_curve.png');
saveas(gcf, rocFile);
close(gcf);
